(function() {
    ...
    var randomCat = function() {
        var chatters = ['jin', 'jack5', 'beanAvatar', 'nina', 'smurfAvatar', 'pikachu', 'siamese', 'korat', 'japanesebobtail', 'abyssinian', 'scottishfold'];
        return chatters[(Math.random() * chatters.length) >>> 0];
    };
    ...
    template.getListWithOnlineStatus = function(list) {
        [].forEach.call(list, function(l) {
            ...
            if (catName === undefined || /\s/.test(l.uuid)) {
                l.uuid = 'fail-cat';
                console.log('...it works in my environment...');
            }
